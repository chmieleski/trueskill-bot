...
tmp="$(mktemp)"
{
  ...
  echo "OBS_ENABLED=${PARAMS[OBS_ENABLED]:-true}"
  echo "OBS_BIND=${PARAMS[OBS_BIND]:-127.0.0.1}"
  echo "OBS_PORT=${PARAMS[OBS_PORT]:-8790}"
  echo "OBS_TOKEN=${PARAMS[OBS_TOKEN]:-}"
  echo "OPS_ALERT_CHANNEL_ID=${PARAMS[OPS_ALERT_CHANNEL_ID]:-}"
  echo "OBS_RSS_MB_WARN=${PARAMS[OBS_RSS_MB_WARN]:-512}"
  echo "OBS_EVENT_LOOP_MS_WARN=${PARAMS[OBS_EVENT_LOOP_MS_WARN]:-200}"
  echo "OBS_SAMPLE_INTERVAL_MS=${PARAMS[OBS_SAMPLE_INTERVAL_MS]:-15000}"
} > "$tmp"

# Append any extra SSM keys not already written (forward-compat for new params).
known=' DISCORD_TOKEN CLIENT_ID GUILD_ID DATABASE_URL DIRECT_URL GEMINI_API_KEY LOG_LEVEL MATCH_CREATE_ROLE_ID MATCH_MOD_ROLE_ID WC3STATS_TIMEOUT_MS API_ENABLED API_PORT API_BIND OBS_ENABLED OBS_BIND OBS_PORT OBS_TOKEN OPS_ALERT_CHANNEL_ID OBS_RSS_MB_WARN OBS_EVENT_LOOP_MS_WARN OBS_SAMPLE_INTERVAL_MS '
