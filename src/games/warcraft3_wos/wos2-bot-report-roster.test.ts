...
const twoPlayerPidFallbackRaw = encodeWos2eExport(
  [
...
const schema2SampleRaw = encodeWos2eExport(
  [
...
describe('botSlotFromReportPlayer', () => {
  ...
  it('falls back to pid when team_slot is absent (team_slot=-1)', () => {
    const report = parseWos2BotReport(twoPlayerPidFallbackRaw);
...
describe('lobbyPlayersFromWos2Report', () => {
  it('builds lobby players via pid fallback when team_slot is null', () => {
    const profile = getGameProfile(WARCRAFT3_WOS_GAME_ID);
    const report = parseWos2BotReport(twoPlayerPidFallbackRaw);
