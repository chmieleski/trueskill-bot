...
**Rules (easy version)**
• One Discord account ↔ one nick for this game, shared across all servers
• If your nick is already linked to someone else, ask a **mod**
• Mods can `/link` or `/unlink` **other people** when something is wrong

Use `/link` in a channel bound to the right league (or pass `league:` if you have more than one).
